...
namespace TJFramework.Extensions
{
  public static class XxRadGridView
  {
    ...
    {// Получим значение ячейки в указанном столбце текущей строки //
      ...
      catch (Exception ex) { TJFramework.Logger.Manager.Log.Save(ex, "ZzGetStringValue"); }
      return s;
    }
    ...
    public static string ZzGetStringValueByFieldName(this RadGridView grid, string FieldName) => ZzGetStringValue(grid, TJStandard.GetGridColumnName(FieldName));

    public static int ZzGetIntegerValueByFieldName(this RadGridView grid, string FieldName, int DefaultValue) => TJConvert.ToInt32(ZzGetStringValue(grid, TJStandard.GetGridColumnName(FieldName)), DefaultValue);
    ...
    public static void ZzFilter(this RadGridView grid, string FieldName, FilterOperator filterOperator, object value, bool ClearPreviousFilter = true)
    {
      ...
      grid.FilterDescriptors.Add(TJStandard.GetGridColumnName(FieldName), FilterOperator.StartsWith, value);
    }
  }
}